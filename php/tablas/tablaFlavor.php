...
$squer="select m.codflavor as codmarca,m.nombre from cat_flavors m inner join tra_cat_prov tc on tc.codcat=m.codflavor where tc.codprov='".$_SESSION['codprov']."' and (m.nombre like '%".$_POST['nombre']."%') order by nombre";				
				if($ejecutar=mysqli_query(conexion($_SESSION['pais']),$squer))
				{
					...
					while($row=mysqli_fetch_array($ejecutar,MYSQLI_ASSOC))
					{
						...
						$contador++;
						echo "<tr onDblClick=\"document.getElementById('flavor').value='".($row['codmarca'])."';setTimeout(function(){\$('#Busqueda').html('');$('#Busqueda').dialog('close');},500);\">
								...
								<td  width='40%'><img src='../../images/editar.png' title='Editar ".$lang[$idioma]['Flavor1']."' id='EditaBusqueda' onClick=\"actualizaExtras('flavor','".$_SESSION['codEmpresa']."','".$_SESSION['pais']."','".$row['codmarca']."');document.getElementById('flavor').value='".($row['codmarca'])."';\" /> </td>
					...
					{
						echo "<tr><td colspan=\"4\"><center><span style=\"font-size:25px;\">".$lang[$idioma]['NoEncontrado1']." ".$lang[$idioma]['Flavor1']."</span></center></td></tr>";
					}
