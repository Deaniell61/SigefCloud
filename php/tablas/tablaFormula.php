...
$squer="select m.codform as codmarca,m.nombre from cat_forms m inner join tra_cat_prov tc on tc.codcat=m.codform where tc.codprov='".$_SESSION['codprov']."' and (m.nombre like '%".$_POST['nombre']."%') order by nombre";				
				if($ejecutar=mysqli_query(conexion($_SESSION['pais']),$squer))
				{
					...
					while($row=mysqli_fetch_array($ejecutar,MYSQLI_ASSOC))
					{
						...
						$contador++;
						echo "<tr onDblClick=\"document.getElementById('formula').value='".($row['codmarca'])."';setTimeout(function(){\$('#Busqueda').html('');$('#Busqueda').dialog('close');},500);\">
								...
								<td  width='40%'><img src='../../images/editar.png' title='Editar ".$lang[$idioma]['Formula']."' id='EditaBusqueda' onClick=\"actualizaExtras('formulas','".$_SESSION['codEmpresa']."','".$_SESSION['pais']."','".$row['codmarca']."');document.getElementById('formula').value='".($row['codmarca'])."';\" /> </td>
