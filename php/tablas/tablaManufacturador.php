...
$squer="select m.codmanufac as codmarca,m.codigo,m.nombre from cat_manufacturadores m inner join tra_cat_prov tc on tc.codcat=m.codmanufac where tc.codprov='".$_SESSION['codprov']."' and (m.nombre like '%".$_POST['nombre']."%' or m.codigo like '%".$_POST['nombre']."%') order by nombre,codigo";				
				if($ejecutar=mysqli_query(conexion($_SESSION['pais']),$squer))
				{
					...
					while($row=mysqli_fetch_array($ejecutar,MYSQLI_ASSOC))
					{
						...
						$contador++;
						echo "<tr onDblClick=\"document.getElementById('manufacturadores').value='".($row['codmarca'])."';setTimeout(function(){\$('#Busqueda').html('');$('#Busqueda').dialog('close');},500);\">
								...
								<td  width='40%'><img src='../../images/editar.png' title='Editar ".$lang[$idioma]['Manufacturadores']."' id='EditaBusqueda' onClick=\"actualizaExtras('manufacturadores','".$_SESSION['codEmpresa']."','".$_SESSION['pais']."','".$row['codmarca']."');document.getElementById('manufacturadores').value='".($row['codmarca'])."';\" /> </td>
					...
					{
						echo "<tr><td colspan=\"4\"><center><span style=\"font-size:25px;\">".$lang[$idioma]['NoEncontrado1']." ".$lang[$idioma]['Manufacturadores']."</span></center></td></tr>";
					}
