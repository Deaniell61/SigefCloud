...
function buscarForm($codigo)
{
	$idioma=idioma();
	include('../idiomas/'.$idioma.'.php');
	$squery="select orderid, timoford, grandtotal, orderunits, ordsou, tranum,codorden,shifee,shicar,firstname,lastname,shipcity,shipstate,(select sum(d.linetotal) from tra_ord_det d where d.codorden=e.codorden ),shitot from tra_ord_enc e where orderid='".$codigo."'";
	$chequeQ = "select concat(TIPOMOV, ' ', NUMERO) as cheque from tra_mob_enc where obser like '%$codigo%';";
	$chequeR = mysqli_query(conexion($_SESSION["pais"]), $chequeQ);
	$cheque = mysqli_fetch_array($chequeR)[0];
			...
			//if($ejecuta->num_rows>0)
			{
				if($row=mysqli_fetch_array($ejecuta,MYSQLI_NUM))
				{
					...
					echo "<script>
										...
										document.getElementById('enMedio').innerHTML='".$row['0']."';
										document.getElementById('numeroTraking').innerHTML='<a href=\'".$link.">".($row['5'])."</a>';
										document.getElementById('numeroCheque').innerHTML='$cheque';
										...
										document.getElementById('totalOrden').innerHTML='".toMoney(round($row['13'],5,2))."<br>".toMoney(round($row['14'],5,2))."<br>".toMoney(round($row['13'],5,2))."';
...
function tablaOrden($codigo,$pais)
{
	...
	include('../idiomas/'.$idioma.'.php');
		...
		echo"<table id=\"tablas\" style=\"text-aling:left;\" border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" class=\"hover tablas table\">
				...
				<tr  class=\"titulo\">
					...
					<th width=\"2%\"></th>
					<th width=\"50%\">". utf8_decode($lang[$idioma]['disnam']) ."</th>
	...
	$ejecutar=mysqli_query(conexion($pais),$squer);
		   ...
		   <script>setTimeout(configuraTabla,100);
