...
{	
	...
	/*	
	$dir=strtolower($dir.strtolower(limpiar_caracteres_especiales(substr($_SESSION['prodName'],0,1)))."/");
	$dirc=strtolower($dirc.strtolower(limpiar_caracteres_especiales(substr($_SESSION['prodName'],0,1)))."/");
	$dir2=strtolower("/".strtolower(limpiar_caracteres_especiales(substr($_SESSION['prodName'],0,1)))."/");
		if(!is_dir($dir)) 
		{
        	mkdir($dir, 0777);
	...
	$dir=strtolower($dir.strtolower(limpiar_caracteres_especiales(substr($_SESSION['prodName'],1,1)))."/");
	$dirc=strtolower($dirc.strtolower(limpiar_caracteres_especiales(substr($_SESSION['prodName'],1,1)))."/");
	$dir2=strtolower($dir2.strtolower(limpiar_caracteres_especiales(substr($_SESSION['prodName'],1,1)))."/");
		if(!is_dir($dir)) 
		{
	        mkdir($dir, 0777);
...
include('../../idiomas/'.$idioma.'.php');
	$cod=sys2015();
		switch($nom)
		{
			default:
			{
				if(!comprobarFILE($codprov,$nombre))
				{
					$sql_auten="insert into documents(coddocto,archivo,descripcion,codprov,tipo) values('$cod','$direc','$nom','$codprov','$tipo')";
				}
				else
				{
					$sql_auten="update documents set archivo='$direc',tipo='$tipo' where codprov='$codprov' and archivo like '$nombre%'";
				}
				break;
			}
		}
...
{	
						...
						echo "<span>".$lang[ $idioma ]['ArchivoGuardado']."</span><script>limpiarArchivosProv();/*llamarProveedor('7','$codprov');*/mostrarArchivos();</script> ";
					
					mysqli_close(conexion($_SESSION['pais']));					
}
else
{
	echo "<script>alert(\"$sql_auten\");</script>";
}
}
#Fin Guardar Imagen Frontal

function comprobarFILE($cod,$nom)
{require_once('../../fecha.php');
	require_once('../../coneccion.php');
$sql_auten="select archivo from documents where codprov='$cod' and archivo like '$nom%'";
