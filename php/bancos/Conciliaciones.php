...
$squery="select cc.codcuen as codigo,cc.numcuen as cuenta,cc.nombre as nombre,(select cb.nombre from cat_banc cb where cb.codbanc=cc.codbanc) as banco from cat_cuen cc";
echo encabezado().
tabla($squery);
	...
	$ejecutar=mysqli_query(conexion($_SESSION['pais']),$squer);
		...
		if($ejecutar)
		{
			while($row=mysqli_fetch_array($ejecutar,MYSQL_ASSOC))
			{
				...
				{
					$retornar.="<tr onclick=\"abrirConciliacion('Conciliaciones', '".$row['codigo']."')\";>
