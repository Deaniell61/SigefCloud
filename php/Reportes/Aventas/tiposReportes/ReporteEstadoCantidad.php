...
$pais=$_SESSION['pais'];
	...
	$squery="select codestvta,codestado,codperi,sum(subtotal),sum(shipping),sum(discount),sum(total),sum(unidades),sum(ordenes),GIFTWRAP,nomestado from tra_ventas_estado where (codperi between '".$inicio."' and '".$fin."') group by codestado order by codestado";
   ...
   $objPHPExcel->
    getProperties()
        ...
        ->setTitle("EstadosPorCantidad")
        ->setSubject("EstadosPorCantidad")
        ->setDescription("Estados Por Cantidad")
...
$objPHPExcel->getSheetCount();//cuenta las pestañas
	...
	if($_SESSION['codEmpresa']=="_4jt03skyy")
	{
		$objPHPExcel->setActiveSheetIndex($o)
			...
			->setCellValue('H'.$i, $lang[ $idioma ]['TotalCant']); 
   ...
   while ($registro = mysqli_fetch_array($resultado,MYSQLI_NUM)) 
   {
	...
	if($_SESSION['codEmpresa']=="_4jt03skyy")
	{
		$objPHPExcel->setActiveSheetIndex($o)
			...
			->setCellValue('D'.$i, toMoney(round($registro['3'],5,2)))
			->setCellValue('E'.$i, toMoney(round($registro['4'],5,2)))
			->setCellValue('F'.$i, toMoney(round($registro['5'],5,2)))
			->setCellValue('G'.$i, toMoney((round($registro['3'],5,2)+round($registro['4'],5,2)-round($registro['5'],5,2))))
			->setCellValue('H'.$i, $registro['7']);
	 $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "C4:".$letra.($i));
	 $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A4:A".($i));
	 $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion2, "B4:B".($i));
	...
	while($periodo2<=$fin)
	{
		...
		$squery2="select sum(ordenes),sum(subtotal),sum(shipping),sum(discount),sum(total),sum(unidades),GIFTWRAP,nomestado from tra_ventas_estado where codperi='".$periodo2."' and codestado='".$registro['1']."' group by codperi";
      ...
      $i++;
   	 ...
   	 //$objPHPExcel->getActiveSheet()->getStyle('H1:'.$letra.($i-1))->getNumberFormat()->setFormatCode('##########################################################'); 
...
header('Content-Disposition: attachment;filename="Estado_Qty.xlsx"');
