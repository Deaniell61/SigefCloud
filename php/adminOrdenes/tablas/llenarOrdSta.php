...
if($nombre==NULL)
{
	$squery="select codstatus,nombre from cat_ord_sta ";
}
else
{
	$squery="select codstatus,nombre from cat_ord_sta where nombre like '%".$nombre."%'";
}
	...
	$ejecutar=mysqli_query(conexion(""),$squer);
				if($ejecutar)
				{
					...
					while($row=mysqli_fetch_array($ejecutar,MYSQLI_NUM))
					{
						...
						$retornar=$retornar."<tr  onClick=\"abrirAdminOrder('OrdSta','".$row['0']."');\" >
