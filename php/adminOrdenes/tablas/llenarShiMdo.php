...
if($nombre==NULL)
{
	$squery="select codshimdo,nombre from cat_shi_mdo ";
}
else
{
	$squery="select codshimdo,nombre from cat_shi_mdo where nombre like '%".$nombre."%'";
}
	...
	$ejecutar=mysqli_query(conexion(""),$squer);
				if($ejecutar)
				{
					...
					while($row=mysqli_fetch_array($ejecutar,MYSQLI_NUM))
					{
						...
						$retornar=$retornar."<tr  onClick=\"abrirAdminOrder('ShiMdo','".$row['0']."');\" >
