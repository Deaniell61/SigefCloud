...
if($nombre==NULL)
{
	 $squery="select codpaymdo,nombre,descripcio from cat_pay_mdo ";
}
else
{
	$squery="select codpaymdo,nombre,descripcio from cat_pay_mdo where nombre like '%".$nombre."%'";
}
	...
	$ejecutar=mysqli_query(conexion(""),$squer);
				if($ejecutar)
				{
					...
					while($row=mysqli_fetch_array($ejecutar,MYSQLI_NUM))
					{
						...
						$retornar=$retornar."<tr  onClick=\"abrirAdminOrder('PayMdo','".$row['0']."');\" >
