...
if($nombre==NULL)
{
	$squery="select codtorden,nombre from cat_torden ";
}
else
{
	$squery="select codtorden,nombre from cat_torden where nombre like '%".$nombre."%'";
}
	...
	$ejecutar=mysqli_query(conexion(""),$squer);
				if($ejecutar)
				{
					...
					while($row=mysqli_fetch_array($ejecutar,MYSQLI_NUM))
					{
						...
						$retornar=$retornar."<tr  onClick=\"abrirAdminOrder('TOrden','".$row['0']."');\" >
