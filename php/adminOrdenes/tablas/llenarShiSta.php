...
if($nombre==NULL)
{
	$squery="select codshista,nombre from cat_shi_sta ";
}
else
{
	$squery="select codshista,nombre from cat_shi_sta where nombre like '%".$nombre."%'";
}
	...
	$ejecutar=mysqli_query(conexion(""),$squer);
				if($ejecutar)
				{
					...
					while($row=mysqli_fetch_array($ejecutar,MYSQLI_NUM))
					{
						...
						$retornar=$retornar."<tr  onClick=\"abrirAdminOrder('ShiSta','".$row['0']."');\" >
