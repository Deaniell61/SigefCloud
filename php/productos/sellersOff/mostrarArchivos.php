					...
					if($ejecutar=mysqli_query(conexion($_SESSION['pais']),$sql_auten))
					{
						if(mysqli_num_rows($ejecutar)>0)
						{
							while($row=mysqli_fetch_array($ejecutar,MYSQLI_ASSOC))
							{
								...
								if($row['descripcion']!="")
								{
									$flota='<div class="globoFlotante"><div class="parrafoFlotante">'.$row['descripcion'].'</div></div>';
								}
								else
								{
									$flota='';
								...
								$ret.= '
										<li class="linkArchivo"><a href="'.strtolower($ruta).'><img src="../../../images/'.strtolower($row['tipo']).'.png" width="50px" >'.strtolower($row['file']).' '.$flota.'</a>';
