...
$sql = "select ts.codprecom as codseller,ts.codcompe,s.nombre as nombre,c.codchan,c.channel as channel,ts.fecha,ts.unidades,ts.preciomin as pmin,ts.preciomax as pmax,ts.shipping,ts.aplica,ts.asin,ts.azsku,ts.azname, ts.upc from tra_pre_com ts inner join cat_sellers s on s.codseller=ts.codcompe inner join cat_sal_cha c on ts.codcanal=c.codchan where ts.codempresa='" . $_SESSION['codEmpresa'] . "' and ts.codprov='" . @$_SESSION['codprov'] . "' and ts.codprod='" . $_SESSION['codprod'] . "' and codprecom='" . $codprecom . "' ";
            ...
            <table>
                ...
                <tr>
                    ...
                    <td><select class='entradaTexto' id="canal"
                                onChange="verificaImportantes('competencia','guardar30');"><?php echo comboCanalSeller($codempresa, $pais); ?></select>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="2">
                        <center><?php echo $lang[$idioma]['Aplica']; ?><input id="aplica"
                                                                              type="checkbox"<?php echo aplica($row['aplica']); ?>>
                        </center>
                    </td>
                </tr>
                <tr>
                    ...
                    <td class="" colspan="2">
                        <center>
                            <input id="guardar30" type="button" class='cmd button button-highlight button-pill' disabled
                                   onClick="guardarSeller('<?php echo $codempresa; ?>','<?php echo $pais; ?>','<?php echo $codprov; ?>','<?php echo $codprod; ?>','<?php echo $codprecom; ?>');opener.buscarSellers();ventana('cargaLoadS',300,400);setTimeout(function(){window.close();},1000);"
                                   value="<?php echo $lang[$idioma]['Guardar']; ?>"/>
                            <input type="reset" class='cmd button button-highlight button-pill'
                                   onClick="envioDeDataSellers('seller');"
